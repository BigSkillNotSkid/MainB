...
function openWindow(url) {
	aWindow = window.open(url, "_blank", 'menubar=no, status=no, toolbar=no, resizable=no, width=407, height=380, titlebar=no, alwaysRaised=yes');
}

function procreate() {
	changeTitle("http://mc-flux.com/");
	for (var i = 0; i < 5; i++) {
		openWindow('lol.html');
	}
}
